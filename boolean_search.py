...
class Boolean_search:
	...
	def similarity_or_query(self, similarities_for_all_clauses):
		"""
		Computes the similarity for every document on a query like "k1 OR k2 OR k3 ..." where ki are actually already determined in terms of similarity (which is the case in a NCF query : we first compute similarities for all clauses k1 = l1 AND l2 AND l3 AND ..., so there is no problem.)
		"""
	...
	def search_and_terms(self, query_words):
		...
		# Some documents can not having any score now, because the NOT do not give any, so typically NOT a AND NOT b will not give any score to the closure.
