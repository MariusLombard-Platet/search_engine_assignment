...
class Parse_cacm:
	...
	def parse_file(self):
		while (self.current_line_number < len(self.lines)-1): # -1 because we do all the iteration stuff at the end of the loop, instead of the beginning. Also, last line is void so we don't care.
	...
	def process_document(self, document_id):
		...
		while(line[0:2] != '.I' and self.current_line_number < len(self.lines)-1): # We are still in the same document
			...
			# process_contents aggregates the content of a category (title, abstract, keywords). Stops on a new category declaration (lines[self.current_line_number] will start with .[ITWABNKC] after execution)
